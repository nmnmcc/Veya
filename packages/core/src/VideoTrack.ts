...
export namespace VideoTrack {
  ...
  ): Effect.Effect<VideoClip.VideoClip<I, IE, IR, OE | VideoResampler.Error, OR>, never, VideoContext> => {
      ...
      return Stream.unwrap(
        Effect.gen(function* () {
          ...
          const resampled = Iterable.map(clips, (clip) => {
            ...
            return Stream.map(encodable, (frame) => VideoFrame.fit(frame, encodable.context.size, context.size));
          });
          const empty: Stream.Stream<VideoFrame, OE | VideoResampler.Error, OR> = Stream.empty;
